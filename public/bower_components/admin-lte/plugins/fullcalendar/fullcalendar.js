...
fc.lang = function(langCode, newFcOptions) {
	...
	// so no way to tell if this is an initialization or a default-adminSetting.
...
// When adminSetting the time, and the moment has an ambiguous time, it then becomes unambiguous.
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	// Responsible for adminSetting rowCnt/colCnt and any other row/col data
...
Grid.mixin({
	...
	documentDragStart: function(ev, ui) {
		...
		if (view.opt('droppable')) { // only listen if this adminSetting is on
			el = $(ev.target);
...
var TimeGrid = Grid.extend({
	...
	renderSelection: function(range) {
		if (this.view.opt('selectHelper')) { // this adminSetting signals that a mock helper event should be rendered
...
var View = fc.View = Class.extend({
	...
	// Handler for unselecting when the user clicks something and the 'unselectAuto' adminSetting is on
	documentMousedown: function(ev) {
		var ignore;
	...
	// `weekNumberCalculation` adminSetting.
...
var BasicView = fcViews.basic = View.extend({
	...
	// Generates an HTML attribute string for adminSetting the width of the week number column, if it is known
...
fcViews.agenda = View.extend({ // AgendaView
	...
	// Generates an HTML attribute string for adminSetting the width of the axis, if it is known
